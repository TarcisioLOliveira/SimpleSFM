...
def kNNMatch(img1, img2, lowes_thresh=0.7):
    ...
    flann2 = cv.FlannBasedMatcher(index_params,search_params)
    if not (des1 is None) and not (des2 is None):
        matches = flann2.knnMatch(des1,des2,k=2)
...
def trainFlannMatch(img, current_descriptors, lowes_thresh=0.7):
    FLANN_INDEX_KDTREE = 1
    index_params = dict(algorithm = FLANN_INDEX_KDTREE, trees = 5)
    search_params = dict(checks=50)   # or pass empty dictionary
    ...
    matches = flann.match(queryDescriptors=img['des'])
